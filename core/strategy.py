...
class ArbitrageStrategy:
    ...
    def generate_signal(self, spread):
        ...
        Returns:
            str: 交易信号，可能的值：
                - 'BUY_SH_SELL_HK': 买上海、卖香港
                - 'BUY_HK_SELL_SH': 买香港、卖上海
                - None: 无信号
        """
        if spread is None:
            return None
        ...
        if spread > self.spread_threshold:
            logger.info(f"生成信号: 买上海、卖香港 (基差: {spread:.2f})")
            return 'BUY_SH_SELL_HK'
        elif spread < -self.spread_threshold:
            logger.info(f"生成信号: 买香港、卖上海 (基差: {spread:.2f})")
            return 'BUY_HK_SELL_SH'
