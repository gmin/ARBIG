...
class ARBIGServiceContainer:
    ...
    def _update_running_mode(self):
        ...
        try:
            ...
            # 完全交易模式：所有核心服务都在运行
            core_services = ['MarketDataService', 'AccountService', 'RiskService', 'TradingService', 'StrategyService']
    ...
    def _start_core_services(self, ctp_status: str) -> bool:
        ...
        if ctp_status in ["FULL", "TD_ONLY"]:
            ...
            # 3. 启动策略服务（需要所有核心服务都启动后）
            self._start_strategy_service()
        else:
...
def main():
    ...
    try:
        ...
        if not args.api_only:
            ...
            # 在单独线程中启动主系统API服务（使用8000端口，避免与web_admin冲突）
            import threading
            from core.api.main_system_api import start_main_system_api

            api_thread = threading.Thread(
                target=start_main_system_api,
                kwargs={"host": "127.0.0.1", "port": 8000, "container": service_container},
                daemon=True
            )
            api_thread.start()
        ...
        # 自动启动完全交易模式
        logger.info("自动启动完全交易模式...")
        result = service_container.start_system()
        if result.success:
            logger.info("✓ 完全交易模式启动成功")
        else:
            logger.warning(f"⚠ 完全交易模式启动失败: {result.message}")
            logger.info("尝试启动演示模式作为备选...")
            demo_result = service_container.start_system_demo_mode()
            if demo_result.success:
                logger.info("✓ 演示模式启动成功（备选方案）")
            else:
                logger.error(f"✗ 演示模式也启动失败: {demo_result.message}")
