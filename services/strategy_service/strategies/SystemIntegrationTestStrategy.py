...
class SystemIntegrationTestStrategy(ARBIGCtaTemplate):
    ...
    def _generate_test_signal(self, tick: TickData):
        ...
        ## 功能说明
        这是策略的核心方法，负责分析市场条件并生成交易信号。
        采用多因子决策模型，结合价格趋势、位置、波动性等因素。
        ...
        signal_trigger = {
            ...
            'position_before': self.pos,
            'exposure_check': {
                'current_position': self.pos,
                'abs_position': abs(self.pos),
                'max_position': self.max_position,
                'at_limit': abs(self.pos) >= self.max_position
            }
        }

        # 🔧 调试：确认代码执行到这里
        self.write_log(f"🔧 DEBUG: 即将执行实时持仓查询")

        # 🔧 安全第一：发出信号前实时查询持仓
        real_position = self._query_real_position()

        # 🔧 调试：确认查询完成
        self.write_log(f"🔧 DEBUG: 持仓查询完成，结果: {real_position}")
        ...
        if real_position != self.pos:
            self.write_log(f"🔄 持仓同步: {self.pos} → {real_position}")
            self.pos = real_position

        # 基于真实持仓进行风控检查
        predicted_position_buy = abs(self.pos + self.trade_volume)
        if predicted_position_buy > self.max_position:
            self.write_log(f"⚠️ 买入将超限，停止交易: 当前={self.pos}, 买入后={predicted_position_buy}, 限制={self.max_position}")
            return
        else:
            # 基于市场分析生成信号
            ...
            trigger_reason = signal_decision['detailed_reason']
        ...
        # 完善信号触发记录
        signal_trigger.update({
            'action': action,
            'reason': reason,
            'detailed_reason': trigger_reason,
            'volume': self.trade_volume,
            'decision_factors': market_analysis
        })

        # 保存触发记录
        self.signal_triggers.append(signal_trigger)

        # 保持最近50个信号记录
        if len(self.signal_triggers) > 50:
            self.signal_triggers = self.signal_triggers[-50:]

        self.write_log(f"📊 生成信号 #{self.signal_count}: {reason}")
        self.write_log(f"   触发原因: {trigger_reason}")
        self.write_log(f"   价格: {current_price:.2f}, 持仓: {self.pos}")
        self.write_log(f"   市场条件: {market_analysis}")
