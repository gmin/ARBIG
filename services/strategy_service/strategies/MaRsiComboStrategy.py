...
class MaRsiComboStrategy(ARBIGCtaTemplate):
    ...
    # on_order 和 on_trade 不需要覆盖，由父类 cta_template 处理
    # 父类会自动调用 on_order_impl 和 on_trade_impl
    ...
    def on_tick_impl(self, tick: TickData):
        ...
        # 🛡️ 基于Tick的实时风控检查
        if self.pos != 0:
            logger.info(f"🛡️ [风控] 收到tick: {tick.last_price:.2f}, pos={self.pos}, short_price={self.short_price:.2f}")
