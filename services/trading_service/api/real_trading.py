...
async def close_position(request: Dict[str, Any]):
    ...
    try:
        ...
        if direction in ['long', 'all'] and position_info['long_position'] > 0:
            ...
            order_id = ctp.send_order(symbol, 'SELL', close_volume, price, order_type, 'CLOSE_TODAY')
            if order_id:
                orders_sent.append({
                    'order_id': order_id,
                    'direction': 'SELL',
        ...
        if direction in ['short', 'all'] and position_info['short_position'] > 0:
            ...
            order_id = ctp.send_order(symbol, 'BUY', close_volume, price, order_type, 'CLOSE_TODAY')
            if order_id:
                orders_sent.append({
                    'order_id': order_id,
                    'direction': 'BUY',
